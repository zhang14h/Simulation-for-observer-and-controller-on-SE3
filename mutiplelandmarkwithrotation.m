...
for i = 1:iter
  ...
  v_t(:,1,i) = 2*[2*rand(1)-1,2*rand(1)-1,2*rand(1)-1].';
  v_delta(:,1,i) = 2*[1*cos(1*i),1*cos(i/2),1*cos(2*i)] ;
  v_o(:,1,i) = 1*[1*cos(i/3),1*sin(i/3),1*cos(i/3)] ;
  delta1_t(i) = 1*pi/90*sin(i);
  delta2_t(i) = 1*pi/90*sin(2*i);
  delta3_t(i) = 1*pi/90*sin(3*i);
  mo1_t(i) = 2*pi/90*sin(i/2);
  mo2_t(i) = 2*pi/90*sin(i/4);
  mo3_t(i) = 2*pi/90*sin(3*i);
...
%     v_s(:,i) = v_t(:,1,i)+0.1*rand(1)+ 0.3.*(TPt(:,i)+0.03*rand(1)-TPs(:,i));
    se3_s(:,:,i) =  se3_t(:,:,i)-inv(SE3_s(:,:,i))*omega(:,:,i)*SE3_s(:,:,i); 
