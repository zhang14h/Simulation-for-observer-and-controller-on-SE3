...
iter = 10000;
for i = 1:iter
  y(:,i) = [7*cos(i); 0*sin(2*i); 1+0*sin(i/2)*cos(3*i);1];
%   y(:,i) = [0.004*i; 0.006*i; 0.1*i;1];
%   if i < 3*iter/10
%      y(:,i) = [0.01*i; 0; 0;1];
%   else if i <7*iter/10
%          y(:,i) = [0.01*3*iter/10-0.02*(i-3*iter/10); 0.01*(i-3*iter/10); 0;1];
%        else
%          y(:,i) = [0.01*3*iter/10-0.02*(4*iter/10)+0.1*(i-7*iter/10); 0.01*(4*iter/10)-0.04*(i-7*iter/10); 0.01*(i-7*iter/10);1];
%        end;
%   end;    
  w1_t(i) =0*pi/500;
  w2_t(i) =0*pi/500;
  w3_t(i) =0*pi/500;
  v_t(:,1,i) = 0*[-2*2*pi/iter*cos(2*i*pi/iter) 2*2*pi/iter*sin(2*i*pi/iter)  1*2*pi/iter*sin(2*i*pi/iter)].';
  v_delta(:,1,i) = 0*[1*cos(i/200),1*cos(i/200),0.5*cos(i/200)] ;
  v_o(:,1,i) = 0*[1*cos(i/3),1*sin(i/3),1*cos(i/3)] ;
  delta1_t(i) = 0.00*pi/200;
  delta2_t(i) = 0.0*pi/200;
  ...
  if w1_t(i) == 0 & w2_t(i) == 0 & w3_t(i) ==0;
     ...
     w_hat_t(:,:,i) = [0 w_t(1,1,i) w_t(2,1,i);-w_t(1,1,i) 0 w_t(3,1,i);-w_t(2,1,i) -w_t(3,1,i) 0]; %lefs-righs invarians coefficiens,so3;
...
%        deltay(:,p,i) = [0*cos(i*p/2) 0*sin(i*p/5) 0*sin(i*p/4) 0].';   
       omega1(:,:,i) = omega1(:,:,i)+0.005*(-error4(:,:,i)*y(:,i)+y(:,i))*transpose(y(:,i));
...
%     plot3(y(1,i),y(2,i),y(3,i),'o','Color','k','MarkerSize',5)
%     hold off  
%     pause(0.05)
...
figure()
 plot3([TPs(1,1),TPs(1,1)+10*TRs(1,1,1)],[TPs(2,1),TPs(2,1)+10*TRs(1,2,1)],[TPs(3,1),TPs(3,1)+10*TRs(1,3,1)],'r');
    ...
    plot3(y(1,1),y(2,1),y(3,1),'o','Color','k','MarkerSize',5)
    hold off  
    pause(0.05)
figure()
